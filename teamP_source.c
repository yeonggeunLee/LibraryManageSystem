...
20,0,0,0,0,0,27,56,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,1,10,0,0,1,10,0,0,
51,0,0,3,167,0,4,87,0,0,4,0,0,1,0,2,9,0,0,2,9,0,0,2,3,0,0,2,3,0,0,
82,0,0,4,44,0,4,138,0,0,1,0,0,1,0,2,9,0,0,
101,0,0,5,141,0,4,143,0,0,5,2,0,1,0,2,3,0,0,2,9,0,0,2,9,0,0,1,9,0,0,1,9,0,0,
136,0,0,6,100,0,4,156,0,0,1,0,0,1,0,2,9,0,0,
155,0,0,7,121,0,4,163,0,0,2,0,0,1,0,2,3,0,0,2,9,0,0,
178,0,0,8,121,0,4,175,0,0,2,0,0,1,0,2,9,0,0,2,9,0,0,
201,0,0,9,0,0,24,181,0,0,1,1,0,1,0,1,97,0,0,
220,0,0,10,148,0,4,219,0,0,3,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,
247,0,0,11,61,0,2,228,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
270,0,0,12,45,0,2,231,0,0,1,1,0,1,0,1,9,0,0,
289,0,0,13,0,0,17,283,0,0,1,1,0,1,0,1,97,0,0,
308,0,0,13,0,0,45,289,0,0,0,0,0,1,0,
323,0,0,13,0,0,13,297,0,0,5,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,
358,0,0,13,0,0,15,308,0,0,0,0,0,1,0,
373,0,0,14,0,0,17,334,0,0,1,1,0,1,0,1,97,0,0,
392,0,0,14,0,0,45,340,0,0,0,0,0,1,0,
407,0,0,14,0,0,13,348,0,0,3,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,
434,0,0,14,0,0,15,358,0,0,0,0,0,1,0,
449,0,0,15,0,0,31,381,0,0,0,0,0,1,0,
};
...

{
   ...
   sqlstm.sqladtp = &sqladt;
   sqlstm.sqltdsp = &sqltds;
   sqlstm.stmt = "select users.user_number ,users.user_name ,users.user_loa\
ns ,users.user_overdue into :b0,:b1,:b2,:b3  from users ,logdata where users.\
user_number=logdata.current_login";
   sqlstm.iters = (unsigned int  )1;
...
struct { unsigned short len; unsigned char arr[12]; } convert_check_number;

        /* varchar convert_check_name[12]; */ 
struct { unsigned short len; unsigned char arr[12]; } convert_check_name;
...

{
    ...
    sqlstm.sqladtp = &sqladt;
    sqlstm.sqltdsp = &sqltds;
    sqlstm.stmt = "select count(*)  ,user_name ,user_number into :b0,:b1,:b\
2  from users where (user_number=:b3 or user_name=:b4) group by user_number,u\
ser_name";
    sqlstm.iters = (unsigned int  )1;
    ...
    sqlstm.sqadto[2] = (unsigned short )0;
    sqlstm.sqtdso[2] = (unsigned short )0;
    sqlstm.sqhstv[3] = (         void  *)&convert_check_number;
    ...
    sqlstm.sqadto[3] = (unsigned short )0;
    sqlstm.sqtdso[3] = (unsigned short )0;
    sqlstm.sqhstv[4] = (         void  *)&convert_check_name;
...

{
            ...
            sqlstm.sqladtp = &sqladt;
            sqlstm.sqltdsp = &sqltds;
            sqlstm.stmt = "select users.user_number into :b0  from users ,l\
ogdata where users.user_number=logdata.current_login";
            sqlstm.iters = (unsigned int  )1;
...

{
            ...
            sqlstm.sqladtp = &sqladt;
            sqlstm.sqltdsp = &sqltds;
            sqlstm.stmt = "select users.user_number ,users.user_name into :\
b0,:b1  from users ,logdata where users.user_number=logdata.current_login";
            sqlstm.iters = (unsigned int  )1;
...

{
   ...
   sqlstm.sqladtp = &sqladt;
   sqlstm.sqltdsp = &sqltds;
   sqlstm.stmt = "select users.user_number ,users.user_name ,logdata.curren\
t_login into :b0,:b1,:b2  from users ,logdata where users.user_number=logdata\
.current_login";
   sqlstm.iters = (unsigned int  )1;
